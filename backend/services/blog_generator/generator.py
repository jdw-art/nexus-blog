...
class BlogGenerator:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_node("revision", self.pipeline.wrap_node("revision", self._revision_node))     # 修订节点
        ...
        # 回调函数的返回值必须包含在path_map的key中，否则抛出异常
        ...
        # 条件边：检查后决定是搜索还是继续到 Questioner（检测到知识空白则继续执行search）
        workflow.add_conditional_edges(
            "check_knowledge",
    ...
    def _planner_node(self, state: SharedState) -> SharedState:
        ...
        if outline and getattr(self, '_interactive', False) and not auto_confirm:
            ...
            # ⚠️调用 LangGraph interrupt 暂停执行图，进行人工交互
